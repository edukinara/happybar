...
export default async function pendingAssignmentsRoutes(
    ...
    async (request, reply) => {
      try {
        ...
        const organizationId =
          (request as any).organization?.id ||
          (request as any).session?.activeOrganizationId
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        const assignment = await PendingAssignmentManager.getPendingAssignment(
          email,
          organizationId
        )
  ...
  fastify.post('/apply-pending-assignments', async (request, reply) => {
    try {
      ...
      try {
        ...
        if (sessionData?.session) {
          ...
          if (setActiveOrgResult.error) {
            console.error(
              '❌ Failed to set active organization via Better Auth:',
              setActiveOrgResult.error
            )
            throw new Error(
              `Better Auth setActiveOrganization failed: ${setActiveOrgResult.error.message}`
            )
          }
        }
        ...
        console.warn(
          'Error details:',
          (sessionUpdateError as Error)?.message || 'Unknown error'
        )
      }
      ...
      return reply.send({
        ...
        applied,
        message: applied
          ? 'Pending assignments applied successfully'
