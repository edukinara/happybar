...
export class PendingAssignmentManager {
  static async storePendingAssignment(
    assignment: Omit<PendingLocationAssignment, 'createdAt' | 'expiresAt'>
  ) {
    const now = new Date()
  ...
  static async getPendingAssignment(
    email: string,
    organizationId: string
  ): Promise<PendingLocationAssignment | null> {
  ...
  static async deletePendingAssignment(
    email: string,
    organizationId: string
  ): Promise<void> {
    const key = `${PENDING_ASSIGNMENT_PREFIX}${email}:${organizationId}`
  ...
  static async applyPendingAssignment(
    userId: string,
    email: string,
    organizationId: string,
    prisma?: any
