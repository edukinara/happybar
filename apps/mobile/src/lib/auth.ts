...
const debugSecureStore = {
  async setItemAsync(key: string, value: string) {
    try {
      console.log(`🔍 SecureStore SET (${Platform.OS}):`, {
        key,
      ...
      console.error(`❌ SecureStore SET error (${Platform.OS}):`, {
        key,
        error,
      })
      throw error
  ...
  async getItemAsync(key: string) {
    try {
      ...
      console.log(
        `${result ? '✅' : '⚠️'} SecureStore GET result (${Platform.OS}):`,
        {
          key,
          hasValue: !!result,
      ...
      console.error(`❌ SecureStore GET error (${Platform.OS}):`, {
        key,
        error,
      })
      return null
  ...
  async deleteItemAsync(key: string) {
    try {
      ...
      console.error(`❌ SecureStore DELETE error (${Platform.OS}):`, {
        key,
        error,
      })
      throw error
