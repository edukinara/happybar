...
export const useCountStore = create<CountStore>()((set, get) => ({
      ...
      createCountSessionWithAPI: async (sessionData) => {
        try {
          ...
          const countAreas: CountArea[] = (apiCount.areas || []).map((area, index) => ({
            id: area.id,
            name: area.name,
            order: area.order,
      ...
      syncSessionWithAPI: async (sessionId) => {
        ...
        try {
          ...
          const formattedAreas: CountArea[] = (apiCount.areas || []).map((area: any, index: number) => ({
            id: area.id,
            name: area.name,
            order: area.order,
      ...
      saveCountItemToAPI: async (item) => {
        try {
          ...
          if (!session.apiId || session.syncStatus !== 'synced') {
            ...
            const updatedSession = get().countSessions.find(s => s.id === item.countSessionId)
          ...
          if (!area) {
            console.warn(`Area ${item.areaId} not found, using first available area`)
          }
      ...
      getUnsyncedSessions: () => {
        return get().countSessions.filter(session => 
          session.syncStatus === 'not_synced' || session.syncStatus === 'sync_failed'
        )
      },
      ...
      resumeBackendSession: async (apiCountId: string) => {
        try {
          ...
          const convertedAreas: CountArea[] = (apiCount.areas || []).map((area: any) => ({
            id: area.id,
            name: area.name,
            order: area.order,
            status: area.status === 'COMPLETED' ? 'completed' as const : 
                   area.status === 'IN_PROGRESS' ? 'in_progress' as const : 
                   'pending' as const,
          ...
          const newSession: CountSession = {
            ...
            totalItems: apiCount.itemsCounted || 0,
            totalVariance: 0,
            syncStatus: 'synced',
          }
          
          set((state) => ({
            countSessions: [newSession, ...state.countSessions.filter(s => s.id !== sessionId)],
            activeSessionId: sessionId,
          }))
      ...
      cleanupStaleLocalSessions: async (backendSessions: CountSession[]) => {
        ...
        try {
          ...
          for (const session of currentSessions) {
            ...
            if (session.syncStatus === 'not_synced' || session.syncStatus === 'sync_failed') {
              continue
            }
            ...
            else if (session.apiId && allBackendApiIds.has(session.apiId) && !activeBackendApiIds.has(session.apiId)) {
              // Find the backend count to get its current status
              const backendCount = recentCountsResponse.data.counts.find(count => count.id === session.apiId)
              if (backendCount && (backendCount.status === 'COMPLETED' || backendCount.status === 'APPROVED')) {
                // Update local session status to match backend
                get().updateCountSession(session.id, {
                  status: backendCount.status === 'APPROVED' ? 'COMPLETED' as const : backendCount.status,
          ...
          if (staleSessions.length > 0) {
            set((state) => ({
              countSessions: state.countSessions.filter(session => !staleSessions.includes(session.id)),
              countItems: state.countItems.filter(item => 
                !staleSessions.includes(item.countSessionId || '')
              ),
      ...
      getAreaProgress: (sessionId) => {
        ...
        const completed = session.areas.filter(area => area.status === 'completed').length
