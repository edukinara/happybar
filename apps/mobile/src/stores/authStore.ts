...
interface User {
  ...
  role: 'owner' | 'admin' | 'manager' | 'inventory_manager' | 'buyer' | 'supervisor' | 'staff' | 'viewer';
}
      ...
      if (sessionData.data?.session && sessionData.data?.user) {
        ...
        if (storedUserData) {
          ...
          userData = {
            ...
            name: sessionData.data.user.name || sessionData.data.user.email.split('@')[0],
            organizationId: (sessionData.data.session as any).activeOrganizationId || '',
