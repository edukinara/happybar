...
export type HappyBarRole = 
  ...
  | 'manager'
  | 'inventoryManager'
  | 'inventory_manager'  // Support both formats
  | 'buyer'
  | 'supervisor'
  | 'staff'
  | 'viewer'
...
export function getRoleDisplayInfo(role: HappyBarRole) {
  const roleInfo = {
    owner: { title: 'Owner', level: 'Executive', color: 'purple', description: 'Full system access' },
    admin: { title: 'Administrator', level: 'Executive', color: 'red', description: 'System administration' },
    manager: { title: 'Manager', level: 'Management', color: 'blue', description: 'Location and team management' },
    inventoryManager: { title: 'Inventory Manager', level: 'Management', color: 'green', description: 'Inventory operations' },
    inventory_manager: { title: 'Inventory Manager', level: 'Management', color: 'green', description: 'Inventory operations' }, // Support snake_case
    buyer: { title: 'Buyer', level: 'Specialist', color: 'orange', description: 'Purchasing and procurement' },
    supervisor: { title: 'Supervisor', level: 'Supervisor', color: 'yellow', description: 'Daily operations oversight' },
    staff: { title: 'Staff', level: 'Staff', color: 'gray', description: 'Standard user access' },
    viewer: { title: 'Viewer', level: 'Read-Only', color: 'slate', description: 'Read-only access' }
  }

  return roleInfo[role] || { title: role, level: 'Unknown', color: 'gray', description: 'Unknown role' }
}
...
export function isLocationRestrictedRole(role: HappyBarRole): boolean {
  return ['inventoryManager', 'inventory_manager', 'supervisor', 'staff', 'viewer'].includes(role)
}
 ...
 */
export function canAccessNavigation(path: string, userRole: HappyBarRole): boolean {
  const requiredPermissions = NAVIGATION_PERMISSIONS[path as keyof typeof NAVIGATION_PERMISSIONS]
