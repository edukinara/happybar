...
import {
  recipesApi,
  type CreateRecipePOSMappingRequest,
  type RecipePOSMappingSearchParams,
  type UpdateRecipePOSMappingRequest,
} from '../api/recipes'
...
export const recipeKeys = {
  ...
  costBreakdown: (id: string) =>
    [...recipeKeys.detail(id), 'cost-breakdown'] as const,
  posMappings: () => [...recipeKeys.all, 'pos-mappings'] as const,
  posMappingsList: (params: RecipePOSMappingSearchParams) =>
    [...recipeKeys.posMappings(), params] as const,
  mappingSuggestions: (integrationId: string) =>
    [...recipeKeys.all, 'mapping-suggestions', integrationId] as const,
}
...
export function useRecipeMappingSuggestions(integrationId: string) {
  return useQuery({
    ...
    retry: (failureCount, error) => {
      console.error(
        `❌ Recipe suggestions API error (attempt ${failureCount}):`,
        error
      )
...
export function useBulkCreateRecipePOSMappings() {
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      ...
      if (failed === 0) {
        toast.success(
          `Successfully created ${successful} recipe mapping${successful !== 1 ? 's' : ''}`
        )
